...
\begin{frame}{A Motivating Example: Route Planning}
  \begin{columns}[T]
    \column{0.55\textwidth}
      \textbf{Problem:} Given $n$ locations, find the shortest round
      trip that visits every location and returns to the start.
      \vspace{0.4em}

      \textbf{Brute-force idea:} enumerate \emph{every} possible route,
      compute its length, and return the shortest.
      \vspace{0.4em}
      \begin{itemize}
        \item $n = 4$: 3 possible routes (shown right)
        \item $n = 5$: 12 possible routes
        \item $n = 6$: 60 possible routes
        \item $n = 10$: $\approx 181\,000$ routes
        \item $n = 20$: $\approx 6\times10^{16}$ routes
      \end{itemize}
    \column{0.40\textwidth}
      \centering
      \textbf{The 3 routes for $n = 4$}
      \vspace{0.3em}

      \begin{tikzpicture}[
        node/.style={circle,draw=PrimaryMid,fill=PrimaryLight!50,minimum size=0.35cm},
        startnode/.style={circle,draw=AlertRed,fill=AlertRed!20,minimum size=0.35cm},
        edge/.style={draw=SuccessGreen,thick},
        divider/.style={draw=gray!50}
      ]
        % Route 1: square cycle
        \node[startnode] (a1) at (0,1) {};
        \node[node] (b1) at (1,1) {};
        \node[node] (c1) at (0,0) {};
        \node[node] (d1) at (1,0) {};
        \draw[edge] (a1)--(b1)--(d1)--(c1)--(a1);
        \draw[divider] (1.5,-0.3)--(1.5,1.3);
        % Route 2: crossed cycle
        \node[startnode] (a2) at (2,1) {};
        \node[node] (b2) at (3,1) {};
        \node[node] (c2) at (2,0) {};
        \node[node] (d2) at (3,0) {};
        \draw[edge] (a2)--(b2)--(c2)--(d2)--(a2);
        \draw[divider] (3.5,-0.3)--(3.5,1.3);
        % Route 3: bowtie cycle
        \node[startnode] (a3) at (4,1) {};
        \node[node] (b3) at (5,1) {};
        \node[node] (c3) at (4,0) {};
        \node[node] (d3) at (5,0) {};
        \draw[edge] (a3)--(c3)--(b3)--(d3)--(a3);
      \end{tikzpicture}
      \begin{alertblock}{General Formula}
        Number of routes $= \dfrac{(n-1)!}{2}$ \quad — checking them
        all is \important{impossible} for large $n$
      \end{alertblock}
  \end{columns}
\end{frame}
    ...
    \column{0.50\textwidth}
      \begin{exampleblock}{Why the iterative version is preferred}
        \begin{itemize}
          ...
          \item Same time complexity $\mathcal{O}(\log \min(m,n))$
        \end{itemize}
      \end{exampleblock}
    \column{0.45\textwidth}
      \begin{block}{Convergence guarantee}
        After two consecutive iterations, the second argument
        decreases by \emph{at least half} — hence $\mathcal{O}(\log \min(m,n))$.
      \end{block}
  \end{columns}
\end{frame}
...
\begin{frame}[shrink=12]{Algorithm 2: Consecutive Integer Checking}
  \begin{columns}[T]
    ...
    \column{0.45\textwidth}
      ...
      \begin{itemize}
        \item $t=24\ldots13$: 12 wasted steps
...
\begin{frame}[shrink=12]{Sieve of Eratosthenes}
  ...
  \begin{columns}[T]
    ...
    \column{0.60\textwidth}
      \textbf{Trace for $n = 25$} (successive passes, $p = 2,3,5$):
      \vspace{0.3em}
      ...
      \resizebox{\linewidth}{!}{%
      \begin{tabular}{*{24}{c}}
        2 & 3 & \cellcolor{SuccessGreen!30}4 & 5 & \cellcolor{SuccessGreen!30}6 & 7 &
        \cellcolor{SuccessGreen!30}8 & 9 & \cellcolor{SuccessGreen!30}10 & 11 &
        \cellcolor{SuccessGreen!30}12 & 13 & \cellcolor{SuccessGreen!30}14 & 15 &
        \cellcolor{SuccessGreen!30}16 & 17 & \cellcolor{SuccessGreen!30}18 & 19 &
        \cellcolor{SuccessGreen!30}20 & 21 & \cellcolor{SuccessGreen!30}22 & 23 &
        \cellcolor{SuccessGreen!30}24 & 25 \\[3pt]
        \textcolor{AlertRed}{\bfseries 2} & 3 & & 5 & & 7 & &
        \cellcolor{AccentYellow!40}9 & & 11 & & 13 & &
        \cellcolor{AccentYellow!40}15 & & 17 & & 19 & &
        \cellcolor{AccentYellow!40}21 & & 23 & & 25 \\[3pt]
        2 & \textcolor{AlertRed}{\bfseries 3} & & 5 & & 7 & & & & 11 & & 13 &
        & & & 17 & & 19 & & & & 23 & & \cellcolor{AccentOrange!40}25 \\[3pt]
        2 & 3 & & \textcolor{AlertRed}{\bfseries 5} & & 7 & & & & 11 & & 13 &
        & & & 17 & & 19 & & & & 23 & & \\
      \end{tabular}}
      \vspace{0.4em}
  ...
  \begin{columns}[T]
    ...
    \column{0.40\textwidth}
      \begin{tikzpicture}[
        ...
        \draw[edge] (a)--(c) node[midway,left,font=\tiny]{5};
        \draw[edge] (a)--(d) node[pos=0.3,above,font=\tiny]{6};
        \draw[edge] (b)--(c) node[pos=0.7,below,font=\tiny]{3};
        \draw[edge] (b)--(d) node[midway,right,font=\tiny]{4};
        \draw[edge] (c)--(d) node[midway,below,font=\tiny]{8};
        \draw[ok] (a)--(b)--(c)--(d)--cycle;
        \node[font=\tiny\bfseries,text=AccentOrange] at (1.5,-0.6)
          {NN tour: a→b→c→d→a = 19};
        \node[font=\tiny,text=SuccessGreen] at (1.5,-1.1)
          {Optimal: a→c→b→d→a = 18};
      \end{tikzpicture}
  \end{columns}
\end{frame}
    ...
    \column{0.55\textwidth}
      ...
      \begin{itemize}
        \item Visit 5 (dist 1), then 3 (dist 2), then 2 (dist 4),
              then 1 (dist 16), back to 4 (dist 21)
        \item Total: \important{44 units}
      \end{itemize}
