...
\begin{frame}[shrink=16]{What is Brute Force?}
  ...
  \begin{columns}[T]
    \column{0.48\textwidth}
      ...
      \begin{itemize}
        ...
        \item \concept{Guaranteed to find a solution} if one exists
      \end{itemize}
    \column{0.48\textwidth}
      \textbf{Disadvantages}
      \begin{itemize}
      ...
      \begin{exampleblock}{When to use it}
        ...
        implementation, or when no better algorithm is known. A baseline for comparing more advanced algorithms.
      \end{exampleblock}
  \end{columns}
\end{frame}
...
\begin{frame}[shrink=12]{Selection Sort}
  \begin{columns}[T]
    ...
    \column{0.45\textwidth}
      ...
      \begin{itemize}
        \item \important{Stable}: Equal elements remain in their relative order.
        \item \important{In-place}: Uses only a constant amount of extra memory.
      \end{itemize}
  \end{columns}
\end{frame}
...
\begin{frame}{Shortest Path via Breadth-First Search}
  \begin{columns}[T]
    ...
    \column{0.52\textwidth}
        ...
        \column{0.68\textwidth}
          \begin{tikzpicture}[
            ...
            nd/.style={circle,draw=PrimaryMid,fill=PrimaryLight!50,
                       font=\bfseries\small,minimum size=0.62cm},
            path/.style={circle,draw=AccentOrange,fill=AccentOrange!25,
                         font=\bfseries\small,minimum size=0.62cm}
          ]
            \node[path] (A) at (0,2.5) {A};
            \node[path] (G) at (1.5,2.5) {G};
            \node[path] (J) at (3.0,2.5) {J};
            \node[nd] (C) at (0.5,1.2) {C};
            \node[nd] (F) at (2.0,1.2) {F};
            \node[nd] (B) at (0,0) {B};
            \node[nd] (D) at (1.5,0) {D};
            \node[nd] (H) at (3.0,0) {H};
            \draw[gray!65] (A)--(G)--(J) (A)--(C)--(F)--(J)
              (A)--(B)--(D)--(H) (G)--(D) (G)--(F);
            \draw[PrimaryMid,very thick,->]
              (A)edge(B) (A)edge(C) (A)edge(G) (B)edge(D)
              (C)edge(F) (G)edge(J) (D)edge(H);
            \draw[AccentOrange,ultra thick,->] (A)--(G)--(J);
          \end{tikzpicture}\par
          \centering\small\textcolor{AccentOrange}{Shortest path: $A\to G\to J$}
        \column{0.29\textwidth}
          \centering\scriptsize
          \textbf{Parents}\par\smallskip
          \begin{tabular}{>{\bfseries}c|c}
            \rowcolor{PrimaryDark}\color{white}Node & \color{white}Parent \\
            \rowcolor{AccentOrange!30} A & -- \\
            B & A \\
            \rowcolor{PrimaryLight!45} C & A \\
            \rowcolor{AccentOrange!30} G & A \\
            D & B \\
            \rowcolor{PrimaryLight!45} F & C \\
            \rowcolor{AccentOrange!30} J & G \\
            H & D
          \end{tabular}
      \end{columns}
      \vspace{0.25em}
      ...
      \centering\scriptsize
      \textbf{Queue trace (front at left)}\par\smallskip
      {\setlength{\tabcolsep}{2.5pt}
      \begin{tabular}{c l|c l}
        \toprule
        \textbf{Processed} & \textbf{Queue} & \textbf{Processed} & \textbf{Queue} \\
        \midrule
        Start & $[A]$       & G & $[D,F,J]$ \\
        A & $[B,C,G]$       & D & $[F,J,H]$ \\
        B & $[C,G,D]$       & F & $[J,H]$ \\
        C & $[G,D,F]$       & \cellcolor{AccentOrange!25}J & \cellcolor{AccentOrange!25}$[H]$; stop \\
        \bottomrule
      \end{tabular}}
  \end{columns}
  \vspace{-0.35em}
